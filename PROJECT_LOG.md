...
### 원인
`getFormData()`(L4554)는 `logs` 배열에서 `currentId`로 항목을 찾아 **그 객체에 폼 값을
채워 넣고 반환**하는 구조인데, 못 찾으면 `null`을 반환했다. `saveAndSync()`는 null이면
"저장할 항목이 없습니다" 토스트만 띄우고 종료 — **화면엔 입력값이 그대로 있는데 저장은
안 되고 결국 통째로 버려진다.**

`logs`/`currentId`가 어긋나는 경로는 여러 개다. 정적 분석 + Playwright 재현으로 추적한
결과 가장 잘 맞는 경로는 **폰에서 작성 중 앱 전환·화면 꺼짐 → iOS가 메모리 확보를 위해
페이지 재로드 → 입력값은 화면에 복원되지만 `currentId` 같은 JS 상태는 초기화**되는 경우.

**배제한 것**: `loadLogsFromCloud()`(L8361)가 `logs`를 통째로 교체하지만, 편집중 보호
가드(폼이 active이고 뷰모드가 아니면 조기 반환)가 **정상 작동함을 실증으로 확인**했다
— 가짜 클라우드 응답을 주입해 폼이 열린 채 재조회가 도착하는 상황을 재현했으나 작성중
항목이 살아남았다. `logs`를 재할당하는 나머지 두 곳(L7165 삭제, L7925 realtime DELETE)은
모두 특정 id만 걸러내므로 작성중 항목과 무관.

### 조치
어느 경로로 어긋났든 **결과가 같으므로 원인별 대응 대신 결과를 막았다.** 폼이 실제로
열려 있는데 항목을 못 찾으면 항목을 되살려 입력 내용을 살린다(이후 코드가 모든 필드를
폼에서 다시 채우므로 골격만 만들면 충분). 복구 발동 시 `console.warn`으로 흔적을 남겨
재발하면 실제 경로를 좁힐 수 있게 했다.

### 검증 (Playwright 실제 재현)
| 시나리오 | 결과 |
|---|---|
| A. `currentId` 소실 (iOS 재로드 상황) | ✅ 행사명·장소 전부 복구 |
| B. `logs` 배열에서 항목만 제거 | ✅ 복구 + 배열 재등록 확인 |
| C. 폼이 안 열린 상태 | ✅ 종전대로 null, 빈 항목 생성 0건 |
| 정상 경로 회귀 | ✅ 로그인→새 일지→입력→저장 정상 |

### 📊 지난 사진 마이그레이션 결과 실측 (2026-08-12 조치의 사후 확인)
| 항목 | 조치 전 | 현재 |
|---|---|---|
| `soundlogs` 전체 크기 | 6,611,877 bytes | **42,502 bytes (155배 감소)** |
| base64 잔존 | 9건 | **0건** |

Storage RLS 수정 후 마이그레이션이 **완전히 완료**됐다. 용량 문제는 해소됐고, 이번
저장 실패는 그와 별개의 상태 불일치 문제였다.

---

## 2026-08-12 ✅ 성공 — 사진 업로드 근본 원인 해결(Storage RLS), 데이터관리 전수조사
### 배경
사장님: "＋새 행사일지 버튼을 눌러도 팝업이 안 뜬다", "알바 기록 저장도 안 된다" →
1차로 `localStorage` quota 초과 크래시를 `safeSetItem` 안전장치로 막았으나 재발.
폰·PC 양쪽에서 "저장공간 부족" 경고를 계속 보게 되자 "데이터관리 책임자로 일해달라,
행사일지 로딩도 느리다"고 요청. Plan 모드로 전환해 Explore 에이전트 3개를 병렬
투입해 전수조사.

### 근본 원인 (실증 확정, 추측 아님)
Supabase Storage 버킷 `event-photos`에 **anon용 SELECT 정책이 없었다.** 앱의 사진
업로드 함수 두 개(`uploadPhotoToStorage`, `uploadEquipPhoto`)가 항상 `x-upsert:true`
헤더를 보내는데, upsert 경로는 기존 파일 존재 확인에 SELECT 권한이 필요하다.

**직접 재현으로 확정**: 완전히 동일한 요청을 `x-upsert` 헤더만 빼면 HTTP 200,
넣으면 매번 `403 new row violates row-level security policy`. 이 기능이 생긴 이후
사진 업로드가 **단 한 번도 성공한 적이 없었다**(soundlogs 23건 전량 조사, Storage URL
형태 사진 0건).

**실측 피해**: `soundlogs`(행사일지) 6.6MB 중 99.7%, `vehicle_data`(장비) 2MB 중
98.8%가 미마이그레이션 base64 사진 + 정리 안 된 레거시 잔재. 이게 로그인마다
로컬에 통째로 미러링되며 브라우저 localStorage quota를 초과시켰고(오늘의 크래시),
탭 전환마다 재다운로드되며 로딩 지연을 유발했다. **세 증상이 전부 한 원인.**

### 조치
1. **Supabase MCP(`apply_migration`)로 `event-photos` 버킷에 anon SELECT 정책 추가.**
   직후 실제 업로드(x-upsert:true) 200 확인, 진단용 테스트파일 삭제까지 정상 작동 확인.
2. 사진 정리 코드(`migrateOldLogPhotos`/`migrateOldEquipPhotos`)는 이미 있었음 — 새로
   안 만들고 그대로 둠. 다음 로그인부터 자동으로 밀린 52건(장비 43+일지 9) 정리 시작.
3. 업로드 실패 시 조용히 `null`만 반환하던 데 `console.warn` 추가, 마이그레이션
   끝난 뒤에도 base64가 남으면 관리자에게만 토스트 알림 — 이번처럼 몇 달간 아무도
   모르고 지나가는 일 재발 방지.

### 보류(임의로 안 건드림, 판단 필요)
- **목록 로딩 경량화**: `loadLogsFromCloud()`가 매번 전체 컬럼을 받는 구조라 손보면
  좋지만, `logs` 배열 자료구조 전체(수십 곳에서 참조)를 건드려야 해서 위험도가 높음.
  근본 원인 수정만으로 payload가 300배(6.6MB→~2만 bytes) 줄 것으로 예상돼, 우선
  마이그레이션 완료 후 재측정하고 그래도 느리면 다시 판단하기로 함.
- **`vehicle_data` 레거시 `id='equipment'` 단일행(334,568 bytes) 삭제**: 현재 앱
  런타임은 이 행을 안 읽어서(→ `eq_*` 행만 읽음) 로컬 성능엔 영향 없지만, 내용
  대조 결과 legacy 안에만 있고 지금 `eq_*` 행엔 없는 장비명이 30개 발견됨(예:
  드럼마이크세트, DI BOX, 무선마이크 수신기 등). 의도적으로 뺀 장비인지 마이그레이션
  누락인지 코드로 구분 불가 — 데이터 손실 위험 있어 삭제하지 않음. **사장님 확인 필요.**

### 검증
- Storage 정책 적용 전/후 동일 요청으로 A/B 재현(전: 403, 후: 200) — 코드 변경 아님,
  실제 인프라 상태로 확정.
- JS 문법 검사, div 태그 균형(989/989) 확인.
- 라이브 배포 확인(v0812-04), `_notifyMigrationIncomplete` 함수 3건(정의+호출2) 반영 확인.
- **미확인(추후 재측정 필요)**: 마이그레이션이 실제로 52건을 다 정리하는지는 다음
  로그인 이후 `soundlogs`/`vehicle_data`를 다시 실측해서 `data:image` 잔존 건수가
  0에 수렴하는지 봐야 한다. 이 세션에서는 인프라 원인 해결까지만 확인했다.
...
**현재 실제 위험도**: 비밀번호는 모든 문서에서 마스킹됐으므로 지금 라이브 문서에 평문 비밀번호는 없다(4개 문서 전부 0회 실측). 다만 `PROJECT_LOG.md`·`CHANGELOG.md`·`.claude/` 같은 내부 문서는 **여전히 공개 서빙 중**이고, 앞으로 민감한 내용을 문서에 적으면 다시 샌다.
...
### 📌 사장님 결정 사항 (재론 금지)
- **강구안 잠금비밀번호 배너는 그대로 둔다.** `index.html`의 로그인화면(`loginScreen`, 1593행)에 있어 로그인 없이 노출되지만, 사장님이 "신경쓰지마"로 명시 판단하셨다(2026-08-05). 의도된 배치이므로 **앞으로 점검에서 이 건을 다시 문제로 올리지 않는다.**
...
- 옛 비밀번호(폐기값, `index.html` 전체 grep에서 미발견)도 CHANGELOG에서 함께 마스킹했다. 이력으로 남길 가치보다 공개 문서에 숫자를 두는 위험이 크다고 판단.
...
### 작업 내용
- **로그인화면**: 로고 아래 "🔒 강구안 잠금비밀번호: ****" 고정 배너 추가(비서가 직접 수정)
